    ...
    <div>
        <div class="perCollectCourses">
            ...
            <el-row v-if="userInfo.collectCourses">
                <el-col
                        :span="33"
                        v-for="(info,index) in userInfo.collectCourses">
                    <el-checkbox
                            v-if="!btnShow"
                            v-model="checked[index]"
                            @change="handleCheckedChange(info.id)">
                    </el-checkbox>
...
<script>
    import {userCollect, userInfo} from "../../api";
    import { mapState } from 'vuex'
    import { collect } from '../../js/func'

    export default {
        name: "PerCollectCourses",
        data() {
            return {
            ...
            * 取消全选： cancelList清空 checked全为false*/
            handleCheckAllChange(val) {
                for (let index in this.userInfo.collectCourses) {
                    if (!this.userInfo.collectCourses.hasOwnProperty(index)) continue;
                    if (val) {
                        this.cancelList.push(this.userInfo.collectCourses[index].id);
